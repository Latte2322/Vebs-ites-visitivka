...
!function(n, e) {
    ...
    function O(n) {
        if (i.rgb(n)) return (t = /rgb\((\d+,\s*[\d]+,\s*[\d]+)\)/g.exec(e = n)) ? "rgba(" + t[1] + ",1)" : e;
        if (i.hex(n)) return (r = n.replace(/^#?([a-f\d])([a-f\d])([a-f\d])$/i, function(n, e, t, r) {
            return e + e + t + t + r + r
        }), a = /^#?([a-f\d]{2})([a-f\d]{2})([a-f\d]{2})$/i.exec(r), "rgba(" + parseInt(a[1], 16) + "," + parseInt(a[2], 16) + "," + parseInt(a[3], 16) + ",1)");
        if (i.hsl(n)) return function(n) {
    ...
    function D(n, e, t) {
        ...
        return u.removeChild(o), r.CSS[e + t] = c * parseFloat(e), c * parseFloat(e)
    }
    ...
    function $(n, e) {
        var t = /[+-]?\d*\.?\d+(?:\.\d+)?(?:[eE][+-]?\d+)?(%|px|pt|em|rem|in|cm|mm|ex|ch|pc|vw|vh|vmin|vmax|deg|rad|turn)?/g,
